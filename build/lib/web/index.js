...
var _session = _interopRequireDefault(require("rqt/build/session"));
...
class Client {
  ...
  constructor(user, password, sandbox = false) {
    ...
    const session = new _session.default({
      headers: {
  ...
  async obtainSession() {
    ...
    const {
      SessionKey
    } = await this.request(u);
    (0, _assert.ok)(SessionKey, `Could not acquire the session key for ${u}.`);
  ...
  async saveSession(session) {
    await (0, _bosom.default)(sessionFile, session.cookie);
  }

  async readSession(session) {
    const cookie = await (0, _bosom.default)(sessionFile);
  ...
  async request(url) {
    ...
    return body; // try to extract token into this.token
  }
